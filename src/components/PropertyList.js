...
function PropertyList() {
  // Use state to store the properties data
  const [properties, setProperties] = useState([])

  // Use useEffect hook to fetch data from sample json
  useEffect(() => {
    // Use fetch or axios to make the request
    fetch('http://localhost:3000/properties')
      .then(response => response.json()) // Parse the response as JSON
      .then(data => setProperties(data)) // Set the properties data in state
      .catch(error => console.error(error)) // Handle any errors
  }, []) // Provide an empty dependencies array to run only once

  // Use map to render each property as a card
  return (
    <div className="container mx-auto px-4">
      {/* Use a grid layout to display the properties */}
      <div class="grid grid-cols-4 gap-4">
        {properties.map(property => (
          <div class="property bg-white shadow-lg rounded-lg overflow-hidden" key={property.id}>
            <div class="property-image">
              {/* Use an image element to show the property image */}
              <img src={property.image} alt={property.title} class="w-full h-48 object-cover"/>
            </div>
            <div class="property-body p-4">
              {/* Use heading and paragraph elements to show the property title, location and house types */}
              <h4 class="property-title text-xl font-bold">{property.title}</h4>
              <p class="property-location text-gray-600">{property.location}</p>
              <p class="property-types text-gray-600">{property.houseTypes}</p>
            </div>
          </div>
        ))}
      </div>
